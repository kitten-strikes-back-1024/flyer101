...
class EnemyPlane(Entity):
    ...
    def fire_missile(self):
        ...
        time_to_target = distance(self.position, plane.position) / 300  # Missile speed
...
def runcamera():
    ...
    camera.position = lerp(
        camera.position,
        desired_pos,
        time.dt * 5    # ← camera follow smoothness (increase = snappier)
    )
...
def update():
    global throttle, lift_force, speed, vertical_velocity, player_health
