...
# F-16 Aircraft Parameters
mass = 12000  # Aircraft mass (kg)
S = 27.87  # Wing area (m²)
wingspan = 9.96  # Wingspan (m)
aspect_ratio = wingspan**2 / S  # Wing aspect ratio

# Aerodynamic Coefficients
CD0 = 0.016  # Zero-lift drag coefficient
k = 1 / (math.pi * aspect_ratio * 0.8)  # Induced drag factor
CL_max = 1.6  # Maximum lift coefficient (stall point)
CL_alpha = 0.095  # Lift curve slope (per degree)
CL0 = 0.1  # Lift coefficient at zero AoA

# Engine
T_max = 127000  # Maximum thrust (N)
T_idle = 12000  # Idle thrust (N)

# Flight state variables
speed = 0  # True airspeed (m/s)
vertical_velocity = 0  # Vertical velocity (m/s)
pitch_rate = 0  # Pitch rotation rate (deg/s)
yaw_rate = 0  # Yaw rotation rate (deg/s)
...
plane_announcement = Audio('models/plane_announcement.mp3', autoplay=True, loop=False)
...
class Missile(Entity):
    def __init__(self, position, forward_vector, velocity, target=None, is_enemy=False, lifetime=10, **kwargs):
        ...
        self.turn_rate = 2  # degrees per frame
...
class EnemyPlane(Entity):
    ...
    def patrol_behavior(self):
        ...
        if distance(self.position, self.patrol_point) < 200:
            ...
            patrol_distance = random.uniform(1500, 3000)
    ...
    def engage_behavior(self):
        ...
        if dot < -0.3:  # We're behind player - move to their side/front
            ...
            target_pos = self.target.position + player_forward * 1500 + perpendicular * 800
            direction = (target_pos - self.position).normalized()
    ...
    def fire_missile(self):
        ...
        time_to_target = distance(self.position, plane.position) / 600  # Missile speed
...
ground = Entity(model='plane', texture='grass', texture_scale=(100, 100), scale=(100000, 100000, 100000), position=((10579.429, 1, 5094.8447)))
...
# HUD
speed_display = Text(text='Speed: 0', position=(-0.7, 0.45), scale=2, color=color.black)
altitude_display = Text(text='Altitude: 0', position=(-0.7, 0.4), scale=2, color=color.black)
throttle_display = Text(text='Throttle: 0%', position=(-0.7, 0.35), scale=2, color=color.black)
distance_display = Text(text='Distance from airport: 0', position=(-0.7, 0.3), scale=2, color=color.black)
aoa_display = Text(text='AoA: 0°', position=(-0.7, 0.25), scale=2, color=color.cyan)
...
radar_grid2 = Entity(parent=camera.ui, model='circle', color=color.rgba(0, 255, 0, 50), scale=0.17, position=(0.65, -0.35), z=-0.4)
radar_grid3 = Entity(parent=camera.ui, model='circle', color=color.rgba(0, 255, 0, 50), scale=0.09, position=(0.65, -0.35), z=-0.4)
...
radar_sweep = Entity(parent=camera.ui, model='quad', color=color.rgba(0, 255, 0, 100), scale=(0.25, 0.002), position=(0.65, -0.35), z=-0.3, rotation_z=0)
...
def input(key):
    ...
    elif key == 'p': autopilot = not autopilot
    elif key == 'space':
    ...
    if key == 'z' and camera_offset.z > 5:
        camera_offset.z -= 2  # Zoom in
    if key == 'x' and camera_offset.z < 25:
...
def get_air_density(altitude):
    """Calculate air density based on altitude using exponential atmosphere model"""
...
def get_lift_coefficient(angle_of_attack):
    """Calculate lift coefficient based on angle of attack (degrees)"""
    # Linear region with stall
    CL = CL0 + CL_alpha * angle_of_attack
    
    # Stall at high angles
    if abs(angle_of_attack) > 15:
        stall_factor = max(0, 1 - (abs(angle_of_attack) - 15) / 10)
        CL = CL * stall_factor
...
def calculate_aerodynamic_forces(velocity, altitude, angle_of_attack, throttle_pct):
    """
    Calculate all aerodynamic forces acting on the aircraft
    Returns: (Lift, Drag, Thrust, Weight) in Newtons
    """
    # Handle zero/low speed case
    if velocity < 0.1:
        return 0, 0, T_idle + (T_max - T_idle) * (throttle_pct / 100), mass * g
    
    # Get air properties
    q = get_dynamic_pressure(velocity, altitude)
    
    # Calculate coefficients
    CL = get_lift_coefficient(angle_of_attack)
    ...
    # Thrust decreases with speed (simplified jet engine model)
    throttle_fraction = throttle_pct / 100
    available_thrust = T_idle + (T_max - T_idle) * throttle_fraction
    thrust_factor = max(0.3, 1 - velocity / 600)  # Thrust falls off at high speed
    Thrust = available_thrust * thrust_factor
...
# Update Loop
def update():
    global throttle, lift_force, speed, vertical_velocity, player_health, angle_of_attack
    ...
    # Calculate aircraft forward vector (where plane is pointing)
    yaw_rad = math.radians(plane.rotation_y)
    pitch_rad = math.radians(plane.rotation_x)
    
    forward_x = math.sin(yaw_rad) * math.cos(pitch_rad)
    forward_y = -math.sin(pitch_rad)
    forward_z = math.cos(yaw_rad) * math.cos(pitch_rad)
    forward_vec = Vec3(forward_x, forward_y, forward_z).normalized()
    
    # Calculate velocity vector (where plane is actually moving)
    if speed > 0.1:
        velocity_x = math.sin(yaw_rad)
        velocity_y = vertical_velocity / max(speed, 0.1)  # Vertical component of velocity
        velocity_z = math.cos(yaw_rad)
        velocity_vec = Vec3(velocity_x, velocity_y, velocity_z).normalized()
        
        # Calculate angle of attack (angle between forward vector and velocity vector)
        # Using cross product to get the angle
        cross_product = forward_vec.cross(velocity_vec)
        cross_length = min(1.0, max(-1.0, cross_product.length()))  # Clamp to [-1, 1]
        angle_of_attack = math.degrees(math.asin(cross_length))
        
        # Determine if AoA is positive or negative based on dot product
        if forward_vec.dot(velocity_vec) < 0:
            angle_of_attack = 180 - angle_of_attack
    else:
        # When stationary, use pitch angle as AoA approximation
        angle_of_attack = plane.rotation_x + 90

    # Use calculated AoA for force calculations
    Lift, Drag, Thrust, Weight = calculate_aerodynamic_forces(angle_of_attack, speed, plane.y, throttle)
    Lift = -Lift
    acceleration = (Thrust - Drag) / mass  
    speed += acceleration * time.dt

    # Move forward in direction plane is pointing
    plane.position += forward_vec * speed * time.dt

    # Vertical movement based on lift
    ...
    aoa_display.text = f'AoA: {angle_of_attack:.1f}°'
    ...
    # Stall Mechanics (using calculated AoA from above)
